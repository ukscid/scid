...
proc configure_style {} {
  ...
  # create toggleswitch if not exists
  if { ! ("Switch.indicator" in [ttk::style element names]) } {
     catch { image delete ::switch(on); image delete ::switch(off) }
     set fg [ttk::style lookup . -foreground "" "#202020"]
     set bg [ttk::style lookup . -background "" #f0f0f0 ]
      # rectangle toggleswitch
      image create bitmap ::switch(on) -foreground $fg -background $bg -data {
          #define switchon_width 22
          #define switchon_height 11
          static unsigned char switchon_bits[] = {
              0xff, 0xff, 0x3f, 0x01, 0x00, 0x20, 0x01, 0xe0, 0x2f, 0x01, 0xe0, 0x2f,
              0x01, 0xe0, 0x2f, 0x01, 0xe0, 0x2f, 0x01, 0xe0, 0x2f, 0x01, 0xe0, 0x2f,
              0x01, 0xe0, 0x2f, 0x01, 0x00, 0x20, 0xff, 0xff, 0x3f };
      }
      image create bitmap ::switch(off) -foreground $fg -background $bg -data {
          #define switchoff_width 22
          #define switchoff_height 11
          static unsigned char switchoff_bits[] = {
              0xff, 0xff, 0x3f, 0x01, 0x00, 0x20, 0xfd, 0x01, 0x20, 0x05, 0x01, 0x20,
              0x05, 0x01, 0x20, 0x05, 0x01, 0x20, 0x05, 0x01, 0x20, 0x05, 0x01, 0x20,
              0xfd, 0x01, 0x20, 0x01, 0x00, 0x20, 0xff, 0xff, 0x3f };
      }
      # circle toggleswitch, rename Rswitch to switch to use them
      image create bitmap ::Rswitch(on) -foreground $fg -background $bg -data {
          #define switchon_width 26
          #define switchon_height 13
          static unsigned char switchon_bits[] = {
              0xe0, 0xff, 0x1f, 0x00, 0x18, 0x00, 0x60, 0x00, 0x04, 0x00, 0x8e, 0x00,
              0x02, 0x80, 0x3f, 0x01, 0x02, 0x80, 0x3f, 0x01, 0x01, 0xc0, 0x7f, 0x02,
              0x01, 0xc0, 0x7f, 0x02, 0x01, 0xc0, 0x7f, 0x02, 0x02, 0x80, 0x3f, 0x01,
              0x02, 0x80, 0x3f, 0x01, 0x04, 0x00, 0x8e, 0x00, 0x18, 0x00, 0x60, 0x00,
              0xe0, 0xff, 0x1f, 0x00 };
      }
      image create bitmap ::Rswitch(off) -foreground $fg -background $bg -data {
          #define switchoff_width 26
          #define switchoff_height 13
          static unsigned char switchoff_bits[] = {
              0xe0, 0xff, 0x1f, 0x00, 0x18, 0x00, 0x60, 0x00, 0xc4, 0x01, 0x80, 0x00,
              0x72, 0x07, 0x00, 0x01, 0x12, 0x04, 0x00, 0x01, 0x19, 0x0c, 0x00, 0x02,
              0x09, 0x08, 0x00, 0x02, 0x19, 0x0c, 0x00, 0x02, 0x12, 0x04, 0x00, 0x01,
              0x72, 0x07, 0x00, 0x01, 0xc4, 0x01, 0x80, 0x00, 0x18, 0x00, 0x60, 0x00,
              0xe0, 0xff, 0x1f, 0x00 };
      }
      ttk::style element create Switch.indicator image \
                 ...
                 -width [expr [image width ::switch(on)] + 2] -sticky w
